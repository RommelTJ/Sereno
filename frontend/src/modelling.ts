...
// state schedule, or staking yield as a legitimate default, and each
// default flatters the plan — untaxed 401(k) draws, a state that
// charges nothing, or no staking income at all — so the screens that
// read them say which effect is off. One place for the copy, since
// Withdrawals and Forecast both show it.
...
export function modellingNotes(warnings: ModellingWarning[], taxYear: number): string[] {
  return warnings.map((warning) => {
    switch (warning) {
      ...
      case 'state_tax_not_modelled':
        return `No state brackets on the ${taxYear} tax year: state tax is modelled at zero.`
